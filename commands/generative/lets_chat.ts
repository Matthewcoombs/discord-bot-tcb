...
const letsChatCommand: Command = {
  ...
  async execute(interaction: ChatInputCommandInteraction) {
    ...
    if (userProfiles.length > 0) {
      ...
      const userProfileChoice = await profileSelectResponse
    ...
    try {
      const collectorFilter = (colMsg: Message) =>
        ...
        (colMsg.author.bot &&
          colMsg.content.includes(interactionTag.toString()));
        ...
        if (
          ...
          const chatCompletionMessages =
            chatCompletionService.formatChatCompletionMessages(
              collected,
              selectedProfile as UserProfile,
            );
      ...
      collector.on('end', async (collected) => {
        ...
        console.log(
          `The chat has been terminated - [interactionTag]: ${interactionTag}`,
        );
      ...
      const errorMessage =
        err?.errorData?.code === USER_TIMEOUT_CODE
          ? err.errorData.error
          : `Sorry there was an error running my chat service!`;
