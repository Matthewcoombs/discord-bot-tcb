...
const directMessageEvent: Command = {
    ...
    async execute(message: Message) {
        const { author: user, channelId } = message;
        const isBot = user.bot;
        const { singleInstanceCommands, chatInstanceCollector } = message.client;
        const userChatInstance = chatInstanceCollector.get(user.id);

        let selectedProfile: UserProfile;
        if (userChatInstance && userChatInstance.channelIds.includes(channelId)) {
            console.log(`user chat instance already set for ${user.username}`);
            return;
        } else {
            selectedProfile = await userProfilesDao.getSelectedProfile(user.id);
            const channelIds: string[] = [...(userChatInstance?.channelIds || []), channelId];
        ...
        console.log('testing chatInstanceCollector init');
